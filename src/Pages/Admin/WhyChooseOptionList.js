...
const WhyChooseOptionList = () => {
  ...
  const handleDelete = (serviceId) => {
    fetch(`http://localhost:5000/delete-why-choose/${serviceId}`, {
      method: "DELETE",
    })
      .then((res) => {
        if (!res.ok) {
          ...
          <div className="row justify-center">
            <div className="col-12">
              <div className="heading-section  fadeInUp style-2 text-center">
                <div className="main-title">Why Choose Us Option</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="container">
        <div class="container">
          ...
          <div class="col-sm flex justify-between gap20">
            <Link
              to={`/admin/edit/why-choose-title/${title._id}`}
              className="tf-button"
              type="submit"
            >
              Update Title
          ...
          <tbody>
            ...
            {choose.map((item, index) => (
              <tr key={item._id}>
                ...
                <td>
                  <Link to={`/admin/edit/why-choose/${item._id}`}>Edit</Link>
                </td>
                <td className="">
                  <button
...
export default WhyChooseOptionList;
